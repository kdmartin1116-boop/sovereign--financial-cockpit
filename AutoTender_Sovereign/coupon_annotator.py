...
def annotate_pdf_coupon(
    ...
    If scale_overlay=True, create an overlay per page sized to the page's MediaBox so placements match page coordinates.
    """
...
def auto_locate_annotations(img_path):
    ...
    # Build annotations by looking for label:value pairs and standalone amounts
    ann = {}
    ...
    for ln, info in lines.items():
        ...
        if m:
            ...
            amount_token = None
            for w, left, top, width, height in info["words"]:
