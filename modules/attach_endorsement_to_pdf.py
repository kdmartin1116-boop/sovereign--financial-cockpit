...
def attach_endorsement_to_pdf_function(
    ...
    try:
        ...
        for i, e in enumerate(endorsement_data.get("endorsements", []), start=1):
            can.drawString(
                50, y, f"{i}. {e.get('endorser_name', 'N/A')} → {e.get('next_payee', 'N/A')}"
            )
            y -= 15
        ...
        can.drawString(
            50, y, f"Signed by: {sig.get('signed_by', 'N/A')} ({sig.get('capacity', 'N/A')})"
        )
        y -= 15
        ...
        if not (0 <= page_index < len(reader.pages)):
            raise ValueError(
                f"Invalid page_index: {page_index}. PDF has {len(reader.pages)} pages."
            )
...
def stamp_pdf_with_endorsement(
    original_pdf_path, output_pdf_path, x, y, endorsement_text, qualifier
):
    try:
