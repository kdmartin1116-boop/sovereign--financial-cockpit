...
class CreditReportParser:
    ...
    def _parse_chex_systems_data(self):
        ...
        chex_section_match = re.search(
            r"Chex Systems Inc\.\s*\((.*?)\)\s*Requested by:\s*(.*?)\s*Requested on:\s*(.*?)\s*Number of Accounts Consumer is Identified On:\s*(\d+)(.*?)(?=Teletrack)",
            self.report_text,
            re.DOTALL,
        )
    ...
    def _parse_teletrack_data(self):
        ...
        teletrack_section_match = re.search(
            r"Teletrack\s*\((.*?)\)\s*Requested by:\s*(.*?)\s*Requested on:\s*(.*?)(.*?)(?=Should you wish to contact TransUnion)",
            self.report_text,
            re.DOTALL,
        )
    ...
    def _parse_fcra_rights(self):
        fcra_rights_match = re.search(
            r"SUMMARY OF RIGHTS\s*GENERAL SUMMARY OF RIGHTS UNDER THE FCRA\s*(.*?)(?=FRAUD VICTIM RIGHTS)",
            self.report_text,
            re.DOTALL,
        )
    ...
    def _parse_fraud_victim_rights(self):
        fraud_victim_rights_match = re.search(
            r"FRAUD VICTIM RIGHTS\s*SUMMARY OF RIGHTS UNDER THE FCRA OF VICTIMS OF IDENTITY THEFT\s*(.*?)(?=--- End of Extracted Text ---|\Z)",
            self.report_text,
            re.DOTALL,
        )
