...
def test_scan_contract(client):
    with patch("os.makedirs"), patch("werkzeug.datastructures.FileStorage.save"):
        data = {"contract": (BytesIO(b"my file contents"), "test.pdf"), "tag": "hidden_fee"}
...
def test_endorse_bill(client):
    with (
        patch("routes.document_routes.PRIVATE_KEY_PEM", "dummy_key"),
        ...
        mock_load_config.return_value = {
            "sovereign_endorsements": [{"trigger": "Test", "meaning": "Test Meaning"}]
        }
        mock_sign.return_value = {
            "signature": "signed",
            "endorser_id": "test-id",
            "endorsement_date": "2025-09-05",
        }
...
def test_stamp_endorsement(client):
    with (
        patch("routes.document_routes.stamp_pdf_with_endorsement") as mock_stamp,
        patch("werkzeug.datastructures.FileStorage.save"),
        patch("os.makedirs"),
    ):
        ...
        with patch("routes.document_routes.send_file") as mock_send_file:
            ...
            response = client.post(
                "/stamp_endorsement", data=data, content_type="multipart/form-data"
            )
...
def test_scan_for_terms(client):
    with (
        ...
        patch("routes.document_routes.PdfReader") as mock_pdf_reader,
    ):  # Patch the PdfReader class in the module
        ...
        mock_pdf_reader.return_value = (
            mock_instance  # Set the return value of the PdfReader constructor
        )

        data = {"file": (BytesIO(b"my file contents"), "test.pdf"), "tag": "hidden_fee"}
