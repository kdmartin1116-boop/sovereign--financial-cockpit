...
def scan_contract():
    ...
    output = (
        f"Scanning contract: {filepath} for tags: {tag}\n"
        f"(pdftotext is installed, but actual scanning logic is not yet implemented)"
    )
    return jsonify({"output": output})


def _handle_bill_upload(request):
    ...
    if not file.filename.lower().endswith(".pdf"):
        return None, jsonify({"error": "Unsupported file type. Please upload a PDF."}), 400
...
def _process_bill_endorsements(filepath, bill_data, sovereign_endorsements):
    endorsed_files = []
    for endorsement_type in sovereign_endorsements:
        ...
        output_pdf_name = (
            f"endorsed_{os.path.basename(filepath).replace('.pdf', '')}_"
            f"{trigger.replace(' ', '')}.pdf"
        )
        endorsed_output_path = os.path.join(os.path.dirname(filepath), output_pdf_name)
...
def endorse_bill():
    ...
    filepath, error_response, status_code = _handle_bill_upload(request)
    if error_response:
    ...
    try:
        ...
        endorsed_files = _process_bill_endorsements(filepath, bill_data, sovereign_endorsements)
...
def _handle_scan_upload(request):
    ...
    if not file or not tag:
        return None, None, jsonify({"error": "Missing file or tag"}), 400
    ...
    return filepath, tag, None, None


def _extract_text_and_scan(filepath, keywords):
    try:
        text = ""
...
def scan_for_terms():
    filepath, tag, error_response, status_code = _handle_scan_upload(request)
    if error_response:
        return error_response, status_code
    ...
    return _extract_text_and_scan(filepath, keywords)
