...
def sign_endorsement(
    endorsement_data, endorser_name, private_key_pem: str = None, private_key_object=None
):
    """
    ...
    if private_key_pem:
        try:
            ...
            raise Exception(f"Error loading private key from PEM string: {e}") from e
    elif private_key_object:
        private_key = private_key_object
    else:
        raise ValueError(
            "Either 'private_key_pem' (as a string) or 'private_key_object' must be provided."
        )

    if not hasattr(private_key, "sign"):
        raise Exception(
            "Provided private key object does not have a 'sign' method (is it an RSA private key?)."
        )
    ...
    try:
        signature = private_key.sign(bytes_to_sign, padding.PKCS1v15(), hashes.SHA256())
    except Exception as e:
        raise Exception(f"Error during signing: {e}") from e
...
if __name__ == "__main__":
    ...
    #         my_endorsement, "John Doe", private_key_pem_path=private_key_file_path
    #     )
    ...
    in_memory_private_key = rsa.generate_private_key(
        public_exponent=65537, key_size=2048, backend=default_backend()
    )

    try:
        my_endorsement_2 = Endorsement(id="456", content="Another test endorsement.")
        signed_endorsement_2 = sign_endorsement(
            my_endorsement_2, "Jane Smith", private_key_object=in_memory_private_key
        )
