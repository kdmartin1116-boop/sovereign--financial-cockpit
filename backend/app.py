from flask import Flask, request, jsonify, render_template, send_file, send_from_directory, url_for, after_this_request
import os
import sys
import yaml
import json
...
from modules.validators import validate_request, rate_limit, InputValidator
...
# Create Flask app with security improvements
...
print("--- Sovereign Financial Cockpit starting with enhanced security ---")
...
def favicon():
    return send_from_directory(os.path.join(app.root_path, '../frontend/public'),
...
@validate_request({
    'bill': {'type': 'file', 'file_type': 'pdf', 'required': True}
})
@safe_file_operation("endorse_bill")
def endorse_bill():
    if not PRIVATE_KEY_PEM:
        raise ConfigurationError("Server is not configured with a private key")

    # Get validated file from request
    ...
    # Use secure filename
    safe_filename = InputValidator.validate_filename(file.filename)
...
@validate_request({
    'bill': {'type': 'file', 'file_type': 'pdf', 'required': True},
    'x': {'type': 'text', 'required': True},
    'y': {'type': 'text', 'required': True},
    'endorsement_text': {'type': 'text', 'required': True, 'max_length': 500},
    'qualifier': {'type': 'text', 'required': False, 'max_length': 100}
})
@safe_file_operation("stamp_endorsement")
def stamp_endorsement_route():
    file = request.files['bill']
    
    # Validate coordinates
    x, y = InputValidator.validate_coordinates(
        request.form.get('x', 0), 
        request.form.get('y', 0)
    )
    
    endorsement_text = InputValidator.validate_text_input(
        request.form.get('endorsement_text', ''), 
        'endorsement_text', 
        max_length=500
    )
    qualifier = InputValidator.validate_text_input(
        request.form.get('qualifier', ''), 
        'qualifier', 
        min_length=0,
        max_length=100
    )
    ...
    safe_filename = InputValidator.validate_filename(file.filename)
    original_filepath = os.path.join(uploads_dir, safe_filename)
    file.save(original_filepath)

    output_filename = f"stamped_{safe_filename}"
...
@log_request
@validate_request({
    'violation': {'type': 'text', 'required': True, 'max_length': 500},
    'jurisdiction': {'type': 'text', 'required': True, 'max_length': 100}
})
def generate_remedy():
    violation = InputValidator.validate_text_input(
        request.form['violation'], 
        'violation', 
        max_length=500
    )
    jurisdiction = InputValidator.validate_text_input(
...
def health_check():
    ...
    try:
        health_status = HealthChecker.get_system_status(app)
...
if __name__ == '__main__':
    app = create_app()
    app.run(host='127.0.0.1', port=8001, debug=app.config.get('DEBUG', False))
